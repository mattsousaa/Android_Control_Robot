...
public class ListaDispositivos extends ListActivity{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if(dispositivosPareados.size() > 0){
            ...
            for(BluetoothDevice dispositivo : dispositivosPareados){
                ...
                String Bt = dispositivo.getAddress();
    ...
    protected void onListItemClick(ListView l, View v, int position, long id) {
        ...
        String enderecoBt = informacaoGeral.substring(informacaoGeral.length() - 17);
        Toast.makeText(getApplicationContext(), "Info: "+ enderecoBt, Toast.LENGTH_SHORT).show();

        Intent retornaBt = new Intent();
        retornaBt.putExtra(ENDERECO, enderecoBt);
        setResult(RESULT_OK, retornaBt);
        finish();
